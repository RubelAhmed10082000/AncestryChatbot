...
def test_generate_html_writes_tree_file(tmp_path):
    nodes = pd.DataFrame(
        [
            {
                "person_id": "person-jane",
    ...
    edges = pd.DataFrame(
        [
            {
                "parent_person_id": "person-george",
                "child_person_id": "person-jane",
    ...
    assert "George Austen" in html_text
    assert "Austen-489" in html_text
    assert "<svg" in html_text
    assert "father_of" in html_text or "solid-line" in html_text


def test_generate_html_handles_empty_nodes(tmp_path):
    output_path = tmp_path / "empty_tree.html"
    ...
    assert output_path.exists()
    assert "No tree data found" in output_path.read_text(encoding="utf-8")
